...
vector<unsigned char> AddRoundKey(vector<unsigned char>& first, vector<unsigned char>& second) { // выполняем побитовое сложение состояния с раундовым ключом
...
unsigned char gmul(unsigned char a, unsigned char b) { // умножение двух байтов в поле Галуа
