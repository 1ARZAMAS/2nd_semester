...
vector<int> findSaddlePoints(const vector<vector<int>>& new_array) {
    ...
    vector<int> rowMin(M, INT_MAX); // INT_MAX это константное значение, которое обозначает макс значение для INT
    for (int i = 0; i < M; ++i) {
        for (int j = 0; j < N; ++j) {
    ...
    vector<int> colMax(N, INT_MIN); // INT_MIN это константное значение, которое обозначает мин значение для INT 
    for (int j = 0; j < N; ++j) {
        for (int i = 0; i < M; ++i) {
